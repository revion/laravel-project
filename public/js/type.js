...
function backAfterDone(param_1, param_2) {
  if(param_1) {
    ...
    $("#modal").modal("toggle")
    $("#type-do-success").empty().show().removeClass("alert-danger").addClass("alert-success").html(message)
    hideAlert()
    onLoad()
  }
}
...
function errorAlert(message) {
  message += "<button class='close' data-hide='alert'>&times;</button>"
  $("#type-add-error").empty().show().html(message)
  hideAlert()
}
///////////////////////////////////////LOADER HTML///////////////////////////////////////////
function onLoad() {
...
function onUpdateModal(id) {
  $.ajax({
    ...
    form +="<div class='form-group'>"
    form += "<label class='form-control sr-only' for='inputTypeName'></label>"
    form += "<input class='form-control' type='text' name='name' placeholder='Type Name' value='"+data[1].name+"' />"
    form += "</div>"
...
function create_one() {
  ...
  if($.trim(name) == "") {
    let error = "<strong>Type name</strong> is required input!"
    errorAlert(error)
    return false
  } else if($.trim(name) == null) {
    let error = "<strong>Type name</strong> must be defined!"
    errorAlert(error)
    return false
  } else if(regex.test($.trim(name))) {
    let error = "<strong>Type name</strong> must be a alphanumeric!"
    errorAlert(error)
    return false
  }
