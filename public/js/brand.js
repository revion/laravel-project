...
function backAfterDone(param_1, param_2) {
  if(param_1) {
    ...
    $("#modal").modal("toggle")
    $("#brand-do-success").empty().show().removeClass("alert-danger").addClass("alert-success").html(message)
    hideAlert()
    onLoad()
  }
}
...
function errorAlert(message) {
  message += "<button class='close' data-hide='alert'>&times;</button>"
  $("#brand-add-error").empty().show().html(message)
  hideAlert()
}
///////////////////////////////////////LOADER HTML///////////////////////////////////////////
function onLoad() {
  $.ajax({
    ...
    var table = "<table border='1' width='100%'><thead><th>No.</th><th>Brand Name</th><th>Action</th></thead><tbody>"
...
function onCreateModal() {
  ...
  let form = "<div class='alert alert-danger alert-dimissible fade show' id='brand-add-error'></div>"
  form +="<div class='form-group'>"
  form += "<label class='form-control sr-only' for='inputTypeName'></label>"
  form += "<input class='form-control' type='text' name='name' placeholder='Brand Name' />"
  form += "</div>"
...
function onUpdateModal(id) {
  $.ajax({
    ...
    let title = "Edit Brand"

    let form = "<div class='alert alert-danger alert-dimissible fade show' id='brand-add-error'></div>"
    form +="<div class='form-group'>"
    form += "<input class='form-control' type='text' name='name' placeholder='Brand Name' value='"+data[1].name+"' />"
    form += "</div>"

    let footer = "<button class='btn btn-inline btn-primary' onclick='update_one(&quot;"+data[1].name+"&quot;, &quot;"+id+"&quot;)'>Update</button>"
    footer += "<button type='button' class='btn btn-danger' data-dismiss='modal'>Close</button>"
...
function onSearch() {
  ...
  $.ajax({
    ...
    if(!data[0]) {
      let message = data[1]
      message += "<button class='close' data-hide='alert'>&times;</button>"
      $("#brand-do-success").empty().show().removeClass("alert-success").addClass("alert-danger").html(message)
      hideAlert()
      table += "<tr>"
      table += "<td colspan='3'>"
      table += "<h4 class='text-center'>No data available</h4>"
...
function create_one() {
  ...
  if($.trim(name) == "") {
    let error = "<strong>Brand name</strong> is required input!"
    errorAlert(error)
    return false
  } else if($.trim(name) == null) {
    let error = "<strong>Brand name</strong> must be defined!"
    errorAlert(error)
    return false
  } else if(regex.test($.trim(name))) {
    let error = "<strong>Brand name</strong> must be a alphanumeric!"
    errorAlert(error)
    return false
  }
